...
def upgrade():
    ...
    # 2) Backfill: gera codigo unico pra cada lanchonete existente.
    # Usa connection direto (Alembic) pra evitar dependencia de ORM aqui.
    conn = op.get_bind()
    ...
    for (lanch_id,) in rows:
        # Loop ate achar codigo unico (chance de colisao desprezivel,
        # mas blindamos com check explicito).
        for _ in range(20):
            cod = _gen_codigo()
            if cod in usados:
                continue
